...
namespace Base {
	...
	Camera3D::Camera3D(const CameraInitialState& config)
	{
		...
		this->fov = config.fov;
		this->aspectRatio = config.aspectRaio;
	...
	void Camera3D::TranslateRelative(const CameraMove direction, const  float velocity)
	{
		...
		else if(direction == CameraMove::RIGHT)
		{
			this->position += this->rightVec * velocity;
		}
		else if(direction == CameraMove::UP)
		{
			this->position += this->upVec * velocity;
		}
		else if(direction == CameraMove::DOWN)
		{
			this->position -= this->upVec * velocity;
		}
	}

	void Camera3D::TranslateAbsolute(const CameraMove direction, const  float velocity)
	{
		if(direction == CameraMove::FORWARD)
		{
	...
	void Camera3D::Rotate(const double yaw, const double pitch)
	{
		this->yaw += yaw;
		this->pitch += pitch;
	...
	void Camera3D::SetRotation(const double yaw, const double pitch)
	{
		this->yaw = yaw;
		this->pitch = pitch;
	...
	{
		glm::vec2 angles = Camera3D::TransformFrontToPitchYaw(front);
	...
	{
		const glm::vec3 front(cos(glm::radians(yaw)) * cos(glm::radians(pitch)), sin(glm::radians(pitch)), sin(glm::radians(yaw)) * cos(glm::radians(pitch)));
	...
	glm::vec2 Camera3D::TransformFrontToPitchYaw(const glm::vec3& front)
	{
